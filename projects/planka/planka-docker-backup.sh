...
DOCKER_CONTAINER_POSTGRES="planka_postgres"
DOCKER_CONTAINER_PLANKA="docker-planka-planka-1"
...
docker run --rm --volumes-from "$DOCKER_CONTAINER_PLANKA" -v "$BACKUP_TEMP:/backup" node:22-alpine cp -r /app/public/user-avatars /backup/user-avatars
docker run --rm --volumes-from "$DOCKER_CONTAINER_PLANKA" -v "$BACKUP_TEMP:/backup" node:22-alpine cp -r /app/public/project-background-images /backup/project-background-images
docker run --rm --volumes-from "$DOCKER_CONTAINER_PLANKA" -v "$BACKUP_TEMP:/backup" node:22-alpine cp -r /app/private/attachments /backup/attachments
echo "Success!"
echo
